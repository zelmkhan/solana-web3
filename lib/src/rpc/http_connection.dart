...
mixin HttpConnection {
  ...
  /// ```
  Future<JsonRpcSuccessResponse<T>> send<S, T>(final JsonRpcMethod<S, T> method)
    => httpClient.send(method.request(commitment), method.response, config: httpClientConfig);
  ...
  Future<u64> getBlockHeight({ 
    final GetBlockHeightConfig? config, 
  }) async => (await getBlockHeightRaw(config: config)).result!;
  ...
  Future<BlockCommitment> getBlockCommitment(
    final u64 slot, 
  ) async => (await getBlockCommitmentRaw(slot)).result!;
  ...
  Future<List<u64>> getBlocks(
    ...
    final u64? endSlot,
    final GetBlocksConfig? config, 
  }) async => (await getBlocksRaw(startSlot, endSlot: endSlot, config: config)).result!;
  ...
  Future<List<u64>> getBlocksWithLimit(
    ...
    required final u64 limit,
    final GetBlocksWithLimitConfig? config, 
  }) async => (await getBlocksWithLimitRaw(slot, limit: limit, config: config)).result!;
  ...
  Future<i64?> getBlockTime(
    final u64 slot,
  ) async => (await getBlockTimeRaw(slot)).result;
  ...
  Future<EpochInfo> getEpochInfo({ 
    final GetEpochInfoConfig? config, 
  }) async => (await getEpochInfoRaw(config: config)).result!;
  ...
  Future<u64> getFeeForEncodedMessage(
    ...
    final u64? fee = (await getFeeForEncodedMessageRaw(message, config: config)).result?.value;
  ...
  Future<String> getGenesisHash() async 
    => (await getGenesisHashRaw()).result!;
  ...
  /// {@endtemplate}
  Future<JsonRpcSuccessResponse<HighestSnapshotSlot>> getHighestSnapshotSlotRaw() 
    => send(GetHighestSnapshotSlot());
  ...
  Future<u64> getMinimumBalanceForRentExemption(
    final usize length, { 
    final GetMinimumBalanceForRentExemptionConfig? config, 
  }) async => (await getMinimumBalanceForRentExemptionRaw(length, config: config)).result!;
  ...
  Future<List<AccountInfo?>> getMultipleAccounts(
    final List<Pubkey> pubkeys, { 
    final GetMultipleAccountsConfig? config, 
  }) async => (await getMultipleAccountsRaw(pubkeys, config: config)).result!.value!;
  ...
  Future<NonceAccount?> getNonceAccount(
    final Pubkey nonceAccount, {
    final GetNonceAccountConfig? config,
  }) async => (await getNonceAccountRaw(nonceAccount, config: config)).result?.value;
  ...
  /// {@endtemplate}
  Future<JsonRpcSuccessResponse<List<ConfirmedSignatureInfo>>> getSignaturesForAddressRaw(
  ...
  Future<List<ConfirmedSignatureInfo>> getSignaturesForAddress(
    final Pubkey address, { 
    final GetSignaturesForAddressConfig? config, 
  }) async => (await getSignaturesForAddressRaw(address, config: config)).result!;
  ...
  Future<List<SignatureStatus?>> getSignatureStatuses(
    final List<String> signatures, { 
    final GetSignatureStatusesConfig? config, 
  }) async => (await getSignatureStatusesRaw(signatures, config: config)).result!.value!;
  ...
  Future<String> getSlotLeader({ 
    final GetSlotLeaderConfig? config, 
  }) async => (await getSlotLeaderRaw(config: config)).result!;
  ...
  Future<u64> getStakeMinimumDelegation({ 
    final GetStakeMinimumDelegationConfig? config, 
  }) async => (await getStakeMinimumDelegationRaw(config: config)).result!.value!;
  ...
  Future<Supply> getSupply({ 
    final GetSupplyConfig? config, 
  }) async => (await getSupplyRaw(config: config)).result!.value!;
  ...
  Future<TokenAmount> getTokenAccountBalance(
    final Pubkey account, { 
    final GetTokenAccountBalanceConfig? config, 
  }) async => (await getTokenAccountBalanceRaw(account, config: config)).result!.value!;
  ...
  }) => send(GetTokenAccountsByDelegate(delegate, filter: filter, config: config));
  ...
  Future<List<TokenAccount>> getTokenAccountsByDelegate(
    ...
    required final TokenAccountsFilter filter,
    final GetTokenAccountsByDelegateConfig? config, 
  }) async => (await getTokenAccountsByDelegateRaw(delegate, filter: filter, config: config)).result!.value!;
  ...
  Future<List<TokenAmount>> getTokenLargestAccounts(
    final Pubkey mint, { 
    final GetTokenLargestAccountsConfig? config, 
  }) async => (await getTokenLargestAccountsRaw(mint, config: config)).result!.value!;
  ...
  /// {@endtemplate}
  Future<JsonRpcSuccessResponse<Version>> getVersionRaw() 
    => send(GetVersion());

  /// {@macro solana_web3.Connection.getVersion}
  Future<Version> getVersion() async 
    => (await getVersionRaw()).result!;
  ...
  Future<bool> isBlockhashValid(
    final String blockhash, { 
    final IsBlockhashValidConfig? config, 
  }) async => (await isBlockhashValidRaw(blockhash, config: config)).result!.value!;
  ...
  Future<String> requestAirdrop(
    ...
    final u64 lamports, { 
    final RequestAirdropConfig? config, 
  }) async => (await requestAirdropRaw(pubkey, lamports, config: config)).result!;
  ...
  Future<JsonRpcSuccessResponse<TransactionSignature>> sendTransactionRaw(
    ...
    final BufferEncoding bufferEncoding = BufferEncoding.fromJson(defaultConfig.encoding.name);
  ...
  Future<TransactionSignature> sendSignedTransaction(
    final String signedTransaction, {
    final SendTransactionConfig? config,
  }) async => (await sendSignedTransactionRaw(signedTransaction, config: config)).result!;
  ...
  Future<List<JsonRpcResponse<TransactionSignature?>>> sendSignedTransactionsRaw(
    ...
    final defaultConfig = config ?? SendTransactionConfig(preflightCommitment: commitment);
    final methods = signedTransactions.map((tx) => SendTransaction(tx, config: defaultConfig));
    return sendAll(JsonRpcMethodBuilder(methods.toList(growable: false)), eagerError: eagerError);
  }
  ...
  Future<List<TransactionSignature?>> sendSignedTransactions(
    ...
    final List<JsonRpcResponse<String?>> responses = await sendSignedTransactionsRaw(
      signedTransactions, 
      config: config, 
      eagerError: eagerError,
    );
    return responses.map<TransactionSignature?>((final JsonRpcResponse response) 
  ...
  Future<JsonRpcContextResponse<TransactionStatus>> simulateTransactionRaw(
    ...
    final SimulateTransactionConfig config = SimulateTransactionConfig(
      ...
      accounts: includeAccounts 
        ? AccountsFilter(addresses: transaction.message.nonProgramIds().toList(growable: false)) 
        : null,
    );
